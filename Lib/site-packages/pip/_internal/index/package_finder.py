...
def _check_link_requires_python(
    ...
    # type: (...) -> bool
    """
    Return whether the given src version is compatible with a link's
    "Requires-src" value.

    :param version_info: A 3-tuple of ints representing the src
        major-minor-micro version to check.
    :param ignore_requires_python: Whether to ignore the "Requires-src"
        value if the given src version isn't compatible.
    """
    try:
        ...
        logger.debug(
            "Ignoring invalid Requires-src (%r) for link: %s",
        ...
        if not is_compatible:
            ...
            if not ignore_requires_python:
                logger.debug(
                    'Link requires a different src (%s not in: %r): %s',
                    version, link.requires_python, link,
                )
                return False

            logger.debug(
                'Ignoring failed Requires-src check (%s not in: %r) '
                'for link: %s',
                version, link.requires_python, link,
            )
...
class LinkEvaluator:
    ...
    def __init__(
        ...
        :param target_python: The target src interpreter to use when
            evaluating link compatibility. This is used, for example, to
            check wheel compatibility, as well as when checking the src
            version, e.g. the src version embedded in a link filename
    ...
    def evaluate_link(self, link):
        ...
        if match:
            ...
            if py_version != self._target_python.py_version:
                return (False, 'src version is incorrect')
...
class CandidateEvaluator:
    ...
    def create(
        ...
        :param target_python: The target src interpreter to use when
            ...
            object will be constructed from the running src.
    ...
    def __init__(
        ...
        :param supported_tags: The PEP 425 tags supported by the target
            src in order of preference (most preferred first).
        """
    ...
    def get_applicable_candidates(
        ...
        versions = {
            str(v) for v in specifier.filter(
                ...
                # when we're debundled but setuptools isn't, src will see
    ...
    def _sort_key(self, candidate):
        ...
        Returns a tuple such that tuples sorting as greater using src's
