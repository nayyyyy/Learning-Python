...
def get_exc_info_from_future(future):
    ...
    On src 2, this will be a (type, value, traceback) triple.
    On src 3, this will be an exception instance (with embedded traceback).

    If there was no exception, None is returned on both versions of src.
    """
    if six.PY3:
