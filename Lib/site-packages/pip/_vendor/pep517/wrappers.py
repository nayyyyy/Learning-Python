...
def norm_and_check(source_tree, requested):
    ...
    # We have to use commonprefix for src 2.7 compatibility. So we
...
class Pep517HookCaller(object):
    ...
    :param python_executable: The src executable used to invoke the backend
    ...
    def _call_hook(self, hook_name, kwargs):
        # On src 2, pytoml returns Unicode values (which is correct) but the
        # environment passed to check_call needs to contain string values. We
        # convert here by encoding using ASCII (the backend can only contain
        # letters, digits and _, . and : characters, and will be used as a
        # src identifier, so non-ASCII content is wrong on src 2 in
        # any case).
